...
def get_mask_token_index(mask_token_id, inputs):
    ...
    mask_token_index = tf.where(inputs['input_ids'][0] == mask_token_id)
    if tf.size(mask_token_index) == 0:
        return None
    return mask_token_index[0][0].numpy()
...
def visualize_attentions(tokens, attentions):
    ...
    for layer_idx in range(num_layers):
        num_heads = len(attentions[layer_idx][0])  # Assuming each layer has the same number of heads
